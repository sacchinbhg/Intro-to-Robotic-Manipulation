...
def generate_edges(n, phi):
    ...
    ans = np.column_stack((x, y, z))

    cone_edges = ans # TODO: Replace None with your result
    # ------------------------------------------------
...
def compute_normals(cone_edges):
    ...
    cross_products = np.cross(cone_edges, np.roll(cone_edges, -1, axis=0))

    facet_normals = cross_products  # TODO: Replace None with your result
    # ------------------------------------------------
...
def compute_minimum_distance_from_facet_normals(a, facet_normals):
    ...
    dots = np.dot(facet_normals, a)

    normal_magnitudes = np.linalg.norm(facet_normals, axis=1)

    dists = np.abs(dots) / normal_magnitudes

    minimum_distance = np.min(dists)  # TODO: Replace None with your result
    # ------------------------------------------------
...
def compute_minimum_distance(a, n, phi):
    ...
    edges = generate_edges(n, phi)

    normals = compute_normals(edges)

    minimum_distance = compute_minimum_distance_from_facet_normals(a, normals)  # TODO: Replace None with your result
    # ------------------------------------------------
...
def check_is_interior_point(a, n, phi):
    ...
    dots = np.dot(normals, a)

    is_interior_point = bool(np.all(dots > 0))  # TODO: Replace None with your result
    # ------------------------------------------------
